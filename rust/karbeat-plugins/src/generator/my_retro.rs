    ...
    fn default() -> Self {
        Self {
            oscillators: smallvec![
                ...
                Oscillator {
                    waveform: Waveform::Triangle, // Classic Nintendo bass pairing
                    detune: -12.0, // One octave down
                    mix: 0.8,
                    pulse_width: 0.5,
                }
            ],
            bitcrush_resolution: 16.0, // Start with crunchy 4-bit/8-bit feel
        }
    }
}
   ...
   fn name() -> &'static str {
                ...
                && midi_events[event_idx].sample_offset as usize == end_frame
            {
                match midi_events[event_idx].data {
                    MidiMessage::NoteOn { key, velocity } => {
                        if velocity > 0 {
                            base.active_voices.push(SynthVoice::new(key, velocity, self.oscillators.len()));
                        } else {
                            for v in base.active_voices.iter_mut() {
                                if v.note == key && v.is_active { v.release(); }
                            }
                        }
                    }
    ...
    fn set_custom_parameter(&mut self, id: u32, value: f32) {
        match id {
            ...
            30 => self.bitcrush_resolution = value.clamp(2.0, 256.0),
            _ => {}
        }
    }
    ...
    fn get_parameter_specs(&self) -> Vec<PluginParameter> {
        let waveforms = vec!["Sine".into(), "Saw".into(), "Square".into(), "Triangle".into(), "Noise".into()];

        vec![
            PluginParameter::new_choice(10, "Waveform", "Oscillator 1", self.oscillators[0].waveform as u32, waveforms.clone(), 2),
            PluginParameter::new_float(11, "Detune", "Oscillator 1", self.oscillators[0].detune, -24.0, 24.0, 0.0),
            PluginParameter::new_float(12, "Mix", "Oscillator 1", self.oscillators[0].mix, 0.0, 1.0, 1.0),
            PluginParameter::new_float(13, "Pulse Width", "Oscillator 1", self.oscillators[0].pulse_width, 0.01, 0.99, 0.5),

            PluginParameter::new_choice(20, "Waveform", "Oscillator 2", self.oscillators[1].waveform as u32, waveforms, 3),
            PluginParameter::new_float(21, "Detune", "Oscillator 2", self.oscillators[1].detune, -24.0, 24.0, -12.0),
            PluginParameter::new_float(22, "Mix", "Oscillator 2", self.oscillators[1].mix, 0.0, 1.0, 0.8),
            PluginParameter::new_float(23, "Pulse Width", "Oscillator 2", self.oscillators[1].pulse_width, 0.01, 0.99, 0.5),

            PluginParameter::new_float(30, "Resolution", "Bitcrush", self.bitcrush_resolution, 2.0, 256.0, 16.0),
        ]
    }
}
...
pub fn create_my_retro_synth(sample_rate: f32, channels: u16) -> RawSynthWrapper<MyRetroEngine> {
